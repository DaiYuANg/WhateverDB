package org.mapdb.core.list;

import org.jetbrains.annotations.NotNull;
import org.mapdb.core.store.Store;
import org.mapdb.core.ser.Serializer;
import org.mapdb.core.ser.Serializers;
...
public class KernelList<E>
  ...
  public static class Maker<E> {
    ...
    public static <E> KernelList.@NotNull Maker<E> newList(@NotNull Store store, Serializer<E> ser) {
  ...
  protected void storeKernel(long @NotNull [] kernel) {
    Arrays.stream(kernel).filter(recid -> recid <= 0).forEach(recid -> {
      throw new ConcurrentModificationException();
    });
